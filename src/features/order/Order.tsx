...
function Order() {
  ...
  function extractItemIngredientsById(id){
    return fetcher?.data?.find(el => el.id === id).ingredients ?? [];
  }
  ...
  return (
    <div className="px-4 py-6 space-y-8">
      ...
      <ul className="divide-y divide-stone-300 border-b border-t">
        {cart.map((item) => {
          return <OrderItem item={item} key={item.pizzaId} ingredients={extractItemIngredientsById(item.pizzaId)} isLoadingIngredients={isLoadingIngredients} />
        })}
      </ul>
